function loginDisApp() {
  ...
  var vUser=encodeURLparam(document.getElementById('app_username').value);
  ...
  saveLocalVar('app_username',vUser);
  ...
  if (vAppStartBoolean) {
    ...
    if (checkMobil()) {
        ...
        openWinHTMLsize(vDBselect+".html?app_submiturl="+vSubmitURL+"&app_username="+vUser+"&app_database="+vDatabase+"&app_email="+vEMail,"500","700");
      } else {
        openWinHTML(vDBselect+".html?app_submiturl="+vSubmitURL+"&app_username="+vUser+"&app_database="+vDatabase+"&app_email="+vEMail);
    };
  } else {
